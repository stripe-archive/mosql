...
EOF
    ...
    it 'Can populate $timestamp on COPY' do
      # If the server isn't running in UTC, this is needed in order to
      # make our NOW() timestamps match up with those generated by the
      # table's NOW() schema.
      Sequel.default_timezone = :localtime
      objects = [
                 {'_id' => "a"},
                 {'_id' => "b"}
                ]
